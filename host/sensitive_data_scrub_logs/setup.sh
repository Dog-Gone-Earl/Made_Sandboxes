...
sudo sed -i.yaml "s/# env: <environment name>/env: log_scrub_box/1" /etc/datadog-agent/datadog.yaml
sudo sed -i.yaml "s/# logs_enabled: false/logs_enabled: true/1" /etc/datadog-agent/datadog.yaml
sudo cp -r /home/vagrant/data/test_logs.d /etc/datadog-agent/conf.d
sudo mv /home/vagrant/data/data.log /var/log
