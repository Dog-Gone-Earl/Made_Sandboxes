...
sudo sed -i.yaml "s/# env: <environment name>/env: journald_box/1" /etc/datadog-agent/datadog.yaml
sudo sed -i.yaml "s/# logs_enabled: false/logs_enabled: true/1" /etc/datadog-agent/datadog.yaml
sudo cp -r /etc/datadog-agent/conf.d/journald.d/conf.yaml.example /etc/datadog-agent/conf.d/journald.d/conf.yaml
